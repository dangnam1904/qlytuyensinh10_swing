	...
	public static PreparedStatement ps;
		 ...
		 KetQuaThiTuyen  kqtt= null;
	        try {
	           ...
	           ResultSet rs= ps.executeQuery();
	            while(rs.next()) {
	             kqtt= new KetQuaThiTuyen(rs.getString("MaHS"), rs.getString("SBD"), rs.getFloat("DiemToan"), rs.getFloat("DiemVan"), rs.getFloat("DiemAnh"), rs.getFloat("DiemMonTC"), rs.getFloat("DiemTong"));
	           }
	        } catch (Exception e) {
	           e.getMessage();
	        }
	 ...
	 public static void insertKQTT(KetQuaThiTuyen hs) {
	   ...
	   public static boolean updateKQTT(KetQuaThiTuyen hs) {
	        try {
	            ps = ConnectDB.getConnect().prepareStatement("update KetquaThituyen set SBD=?,diemtoan=?,diemvan=?,diemanh=?, diemmontc=?,diemtong=? where MaHS=?");
	   ...
	   public static boolean deleteKQTT(String maHS) {
	        try {
	            ps = ConnectDB.getConnect().prepareStatement("DELETE FROM KetquaTuyenSinh WHERE MAHS = ?");
	            ps.setString(1, maHS);
	            return ps.executeUpdate() >0;
	        } catch(Exception e) {
	            return false;
