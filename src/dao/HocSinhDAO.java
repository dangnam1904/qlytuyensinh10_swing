...
public class HocSinhDAO {
	           ...
	           ResultSet rs= ps.executeQuery();
	            while(rs.next()) {
	             hs= new HocSinh(rs.getString("MaHS"), rs.getString("SBD"), rs.getString("MaTruongC2"),
	            		 ...
	            		 rs.getString("HL9"), rs.getString("ChuThich"), rs.getBoolean("gioitinh"),rs.getBoolean("KhoiChuyen"),
	            		 rs.getInt("DiemCong"), rs.getFloat("DiemHK9"), rs.getDate("NgaySinh"));
	           }
	        } catch (Exception e) {
	           e.getMessage();
	        }
	 ...
	 public static void insertDataHocSinh(HocSinh hs) {
	   ...
	   public static boolean updateHocSinh(HocSinh hs) {
	        try {
	            ps = ConnectDB.getConnect().prepareStatement("update HocSinh set SBD=?, MaTruongC2=?, Hoten= ?, NgaySinh= ?, DiaChi=?,"
