...
public class HocSinhView extends JFrame{
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					HocSinhView frame = new HocSinhView();
...
//			
			...
			contentPane.add(rdio_nu);
			  bg = new ButtonGroup();
		        ...
		        String []colsName = {"MaHS","SBD","MaTruongC2","Hoten","NgaySinh","DiaChi","GioiTinh","KhoiChuyen","DiemCong","NV1","NV2","DiemHK9","HL9","ChuThich"};
...
//				table.getColumnModel().getColumn(13).setPreferredWidth(300);		
		...
		public void insertDataHocSinh() {
			if(txt_mahs.getText().trim().equals("") || txt_sbd.getText().trim().equals("")) {
				 ...
				 * DateFormat df = new SimpleDateFormat("MM/dd/yyyy"); Date startDate; String
				 * newDateString=""; try { startDate = (Date)
				 * df.parse(txt_ngaysinh.getText().trim()); newDateString =
			    ...
			    try {
			    	HocSinh hS= new HocSinh(txt_mahs.getText().trim(), txt_sbd.getText().trim(),matruong, txt_hoten.getText().trim(),
							txt_diachi.getText().trim(), ((TruongC3) cb_nv1.getSelectedItem()).getMaTruongC3()+"", ((TruongC3) cb_nv1.getSelectedItem()).getMaTruongC3()+"", 
							txt_hocluc.getText().trim(), txt_ghichu.getText().trim(), gioitinh, khoiChuyen, Integer.valueOf(txt_diemcong.getText().trim()), Float.valueOf(txt_diemtb.getText().trim()),  data);
